...
if (x, y) != (-1, -1):
    ...
    for i in range(n):
        ...
        while not game_start:
            ...
            # Champion Select
            st = time.time()
            while find_target('template/heim.jpg') == (-1, -1) and time.time() - st < 15:
                time.sleep(0.5)
            if find_target('template/heim.jpg') != (-1, -1):
                print('Champion Selection')
                x, y = find_target('template/heim.jpg')
                P.mouseDown(x, y, button = 'left')
                P.mouseUp(x, y, button = 'left')
                ...
                st = time.time()
                # Loading
                while find_target('template/loading.jpg') == (-1, -1) and time.time() - st < 100:
                    time.sleep(0.5)
